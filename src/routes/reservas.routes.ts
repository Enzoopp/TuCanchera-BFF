...
router.get('/', requireAuth, async (req, res) => {
  try {
    ...
    console.error('[BFF] Error inesperado en GET /reservas:', err)
...
router.post('/', requireAuth, async (req, res) => {
  try {
    ...
    console.error('[BFF] Error inesperado en POST /reservas:', err)
    res.status(500).json({ error: 'Error interno del servidor' })
  }
})
...
router.post('/:id/cancelar', requireAuth, async (req, res) => {
  try {
    ...
    console.error('[BFF] Error inesperado en /cancelar:', err)
    res.status(500).json({ error: 'Error interno del servidor' })
  }
})
